...
class Translator(DmlVisitor):
    def __init__(self):
        ...
        self.FUNCTIONS_MAP = {
            ...
            "colIndexMax": self.colIndexMax_function,
            "rowIndexMax": self.rowIndexMax_function,
            "colIndexMin": self.colIndexMax_function,
            "rowIndexMin": self.rowIndexMax_function,
            "cumSum": self.cumSum_function,
            "cumsum": self.cumSum_function,
            "cumProd": self.cumProd_function,
            "cumMin": self.cumMin_function,
            "cumMax": self.cumMax_function,
            "colSums": self.colSums_function,
            "rowSums": self.rowSums_function,
            "colMeans": self.colMeans_function,
            "rowMeans": self.rowMeans_function,
            ...
            # "order": self.order_function,
            "solve": self.solve_function,
            "abs": self.abs_function,
            "exp": self.exp_function,
            "round": self.round_function,
            "floor": self.floor_function,
            "ceil": self.ceil_function,
            "sin": self.sin_function,
            "cos": self.cos_function,
    ...
    def rand_function(self, arguments):
        args, exprs = self.reorder_args(arguments, ["rows", "cols", "min", "max", "sparsity", "seed"])
        function_call = f"rand({args[0]}, {args[1]}, {args[2]}, {args[3]}, {args[4]}, {args[5]})"
        dtype = self.inferType(args[0], exprs[0]);

        return function_call, dtype

    def sample_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None, None, None, None])
        function_call = f"sample({args[0]}, {args[1]}, {args[2]}, {args[3]})"
        dtype = self.inferType(args[0], exprs[0]);
    ...
    def length_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"as.si64(ncells({args[0]}))"
        dtype = "si64"
    ...
    def colIndexMax_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"idxMax({args[0]}, 1)"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def rowIndexMax_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"idxMax({args[0]}, 0)"
        dtype = self.inferType(args[0], exprs[0])
    ...
    def cumSum_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"cumSum({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def cumProd_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"cumProd({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def cumMin_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"cumMin({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def cumMax_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"cumMax({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
    ...
    def colMeans_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"mean({args[0]}, 1)"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def rowMeans_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"mean({args[0]}, 0)"
        dtype = self.inferType(args[0], exprs[0])
    ...
    def median_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"mean({args[0]})"
        dtype = "f64"
    ...
    def rbind_function(self, arguments):
        ...
        # order does not work as intended as it can have a variable size of input parameters
        # def order_function(self, arguments):
        #     args, exprs = self.reorder_args(arguments, [None, None, None, None])
        #     function_call = f"order({args[0]}, {args[1]}, {args[2]}, {args[3]})"
        #     dtype = self.inferType(args[0], exprs[0])
        #     
        #     return function_call, dtype
    ...
    def round_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"round({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def floor_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"floor({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def ceil_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"ceil({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def sin_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"sin({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
        
        return function_call, dtype

    def cos_function(self, arguments):
        args, exprs = self.reorder_args(arguments, [None])
        function_call = f"cos({args[0]})"
        dtype = self.inferType(args[0], exprs[0])
    ...
    def inferType(self, source, ctx):
        ...
        if source_type is not None:
            ...
            if ctx is not None and hasattr(ctx, 'start'):
                line = ctx.start.line
                print(f"inferType error on line {line} in source code")
